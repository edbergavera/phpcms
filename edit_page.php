...
/*if (!$user->canEditPage(new Page($id))) {
   header("Location:index.php");
   exit;
}*/
...
$select_query = 'SELECT id, creatorId, title, content, DATE_FORMAT(dateAdded, "%e %M %Y") AS dateAdded FROM pages WHERE id=:id';
$stmt = $pdo->prepare($select_query);
$select_result = $stmt->execute(array(':id'=>$_GET['id']));
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){ //handle the form submission
    ...
    if($form->validate()){
        ...
        $insert_result = $stmt->execute(array(':id'=>$pid->getValue(),':title'=>$title->getValue(),':content'=>$content->getValue()));
